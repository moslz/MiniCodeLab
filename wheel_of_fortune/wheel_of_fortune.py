...
class WOFHumanPlayer(WOFPlayer):
    ...
    def getMove(self, category, obscuredPhrase, guessed):
        prompt = (
            ...
            f"Category: {category}\n"
            f"Phrase: {obscuredPhrase}\n"
            f"Guessed: {', '.join(guessed)}\n"
            f"Guess a letter, phrase, or type 'exit' or 'pass': "
...
class WOFComputerPlayer(WOFPlayer):
    ...
    def getPossibleLetters(self, guessed):
        possible_letters = [letter for letter in 'ABCDEFGHIJKLMNOPQRSTUVWXYZ' if letter not in guessed]
        
        if self.prizeMoney < self.VOWEL_COST:
            possible_letters = [letter for letter in possible_letters if letter not in self.VOWELS]
...
num_human = getNumberBetween('How many human players? within (Max 4): ', 0, 4)

# Create the human player instances
human_players = [WOFHumanPlayer(input('Enter the name for human player: {} '.format(i+1))) for i in range(num_human)]

num_computer = getNumberBetween('How many computer players?(Max 4): ', 0, 4)
